...
import { editUserProfile,changeProfilePic } from '@/actions/server';
...
function EditProfile() {
  ...
  const saveProfilePic  = async(e)=> {
    e.preventDefault()
    const imageUrl = window.localStorage.getItem("image-url");
    if((uploadedPic || imageUrl) && email){
      const res = await changeProfilePic(imageUrl);
      if (res.status === 201){
      ...
      toast.error("Make sure the upload is complete")
    }
  }
  return (
    <DefaultLayout>
      <div className="mx-auto max-w-270">
        <Breadcrumb pageName={'profile'} additonalRoute={'edit'} />
        ...
        <div className="grid grid-cols-5 gap-8">
                  ...
                  <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                    ...
                    <div className="w-full sm:w-1/2">
                      <label className="mb-3 block text-sm font-medium text-black dark:text-white" htmlFor="phoneNumber" >
                        Phone Number {loading &&  <div className="cssLoader"></div>}
                      </label>
                      <input {...register("phoneNumber",{required:{value: /^\d{10,10}$/,message:"Enter phone Number as per the format"}})}  className="w-full rounded border border-stroke bg-gray px-4.5 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" type="text" name="phoneNumber" id="phoneNumber" placeholder="Phone Number" />
                    </div>
                  </div>

                  <div className="mb-5.5">
                    <label className="mb-3 block text-sm font-medium text-black dark:text-white" htmlFor="emailAddress" >
                      Email Address {loading &&  <div className="cssLoader"></div>}
                    </label>
                    <div className="relative">
                      <span className="absolute left-4.5 top-4">
                        <svg className="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" >
                      ...
                      <input disabled className="cursor-not-allowed w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 text-gray-100 focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" type="email" name="emailAddress" id="emailAddress" placeholder="email" defaultValue={email ? email : ""}/>
                    </div>
                  </div>

                  <div className="mb-5.5">
                    <label className="mb-3 block text-sm font-medium text-black dark:text-white" htmlFor="Username">
                      BIO {loading &&  <div className="cssLoader"></div>}
                    </label>
                    <div className="relative">
                      <span className="absolute left-4.5 top-4">
                        <svg className="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" >
                      ...
                      <textarea {...register("bio")} className="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" name="bio" id="bio" rows={6} placeholder="Write your bio here" ></textarea>
                    </div>
                  </div>
          ...
          <div className="col-span-5 xl:col-span-2">
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              ...
              <div className="p-7">
                <form onSubmit={(e)=>saveProfilePic(e)}>
                  <div className="mb-4 flex items-center gap-3">
                    ...
                    <div>
                      <span className="mb-1.5 text-black dark:text-white">
                        Edit your photo {loading &&  <div className="cssLoader"></div>}
                      </span>
                      <span className="flex gap-2.5">
                        <p className={uploadedPic ? "block text-green-500" : "hidden"}>Click on Save Photo to finish</p>
                  ...
                  <UploadDropzone endpoint="imageUploader" onClientUploadComplete={(res) => {
                        setUploadedPic(res[0].url);
                        window.localStorage.setItem("image-url",res[0].url);
