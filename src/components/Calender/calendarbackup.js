...
const Calendar = () => {
  ...
  function currentMonthGrouping(year, month) {
    const daysInMonth = monthDays[month];
    const daysInCurrentMonth = (month === 1 && isLeapYear(year)) ? 29 : daysInMonth;
    ...
    for (let date = 1; date <= daysInCurrentMonth; date++) {
      ...
      daysGroupedByWeekday[weekdayName].push(date);
    }
  
    return daysGroupedByWeekday;
  }
  
  // Example usage for July 2024
  const year = 2024;
  const month = 6; // July (0-based index, so 6 is July)
  const groupedDays = currentMonthGrouping(year, month);
