...
function Warning2({message,onRequestClose,action,operation}) {
  return (
    ...
    <div className="relative w-full max-w-142.5 rounded-lg bg-warning px-8 py-12 text-center md:px-17.5 md:py-15">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" fill="000" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-12">
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
</svg>

      </span>
      <h3 className="mt-5.5 pb-2 mb-4 text-xl font-bold text-black sm:text-4xl">{operation ? operation : "Irreversable "} operation!</h3>
      <p className="mb-7.5 text-black">{message}</p>
      <div className='flex justify-around'>
        <button onClick={()=>action(0)} className="inline-block rounded  px-12.5 py-3 text-center font-medium text-white transition hover:bg-white hover:text-black bg-rose-600">Yes, {operation ?  operation : "Delete"}</button>
        <button onClick={()=>onRequestClose()} className="inline-block rounded  px-12.5 py-3 text-center font-medium text-white transition hover:bg-white hover:text-black bg-slate-800">Cancel</button>
      </div>
