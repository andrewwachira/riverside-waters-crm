...
function Error({message,onRequestClose,}) {
  return (
    ...
    <div className="relative w-full max-w-142.5 rounded-lg bg-danger px-8 py-12 text-center md:px-17.5 md:py-15">
      ...
      <h3 className="mt-5.5 pb-2 mb-4 text-xl font-bold text-white sm:text-4xl">Error!</h3>
      <p className="mb-7.5 text-white">{message}</p>
      <button onClick={()=>onRequestClose()} className="inline-block rounded border border-white px-12.5 py-3 text-center font-medium text-white transition hover:bg-white hover:text-black">Try again later?</button>
